...
const Posts = () => {
  ...
  const {
    ...
    queryKey: ["posts"],
    queryFn: async () => {
      const res = await axiosInstance.get(`/all-posts?status=approved&limit=8`);
      return res.data;
    },
  });
  ...
  if (isLoading)
    return (
      <div className="text-center my-20 font-bold text-[#3C5D50]">
        Loading Posts...
      </div>
    );
  if (isError)
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 my-16">
      ...
      <Link
        to="/all-posts"
        className="block text-center mx-auto  py-2 w-[150px] rounded-lg mt-12 bg-[#3C5D50] text-white font-semibold hover:opacity-90 transition"
      >
        View More Posts
      </Link>
